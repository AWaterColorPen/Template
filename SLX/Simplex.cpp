...
struct SIMPLEX {
	...
	void pivot(int l, int e)
	{
		...
		b[e] = b[l] / A[l][e];
		A[e][l] = 1.0 / A[l][e];
		for (i=1; i<=n; i++) if (y=N[i], y!=e) A[e][y] = A[l][y] / A[l][e];

		for (j=1; j<=m; j++)
			if (x=B[j], x!=l)
			{
				b[x] = b[x] - A[x][e] * b[e];
				for (i=1; i<=n; i++) if (y=N[i], y!=e) A[x][y] = A[x][y] - A[e][y] * A[x][e];
				A[x][l] = -A[x][e] * A[e][l];
			}

		v += b[e] * c[e];
		c[l] = - A[e][l] * c[e];
		for (i=1; i<=n; i++) if (y=N[i], y!=e) c[y] = c[y] - A[e][y] * c[e];

		for (i=1; i<=n; i++) if (N[i]==e) N[i]=l;
		for (j=1; j<=m; j++) if (B[j]==l) B[j]=e;
	}
	...
	int init()
	{
		int i, j, x, y, l=0;
		for (i=1; i<=n; i++) N[i]=i;
		for (j=1; j<=m; j++) B[j]=n+j;
		v=0;

		for (j=1; j<=m; j++) if (l==0 || b[B[j]]<b[l]) l=B[j];
		if (sgn(b[l])>=0) return ANS_OK;

		memcpy(OC, c, sizeof(OC));
		memset(c, 0, sizeof(c));
		c[0]=-1; N[++n]=0;
		for (j=1; j<=m; j++) A[B[j]][0]=-1;
		...
		for (j=1; j<=m; j++)
			...
			{
				v += c[x] * b[x];
				for (i=1; i<=n; i++) y=N[i], c[y] -= A[x][y] * c[x];
				c[x]=0;
			}
		return ANS_OK;
	}
