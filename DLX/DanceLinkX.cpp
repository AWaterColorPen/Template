...
struct DLX {
	...
	int dfs(int n)
	{
		...
		{
			ANS[n]=i;
			for (j=R[i]; j!=i; j=R[j]) remove_exact(COL[j]);
			if (dfs(n+1)) return 1;
			for (j=L[i]; j!=i; j=L[j]) resume_exact(COL[j]);
		}
	...
	int dfs(int n)
	{
		...
		int i, j, now=inf, c;
		for (i=R[0]; i; i=R[i]) if (now>S[i]) now=S[c=i];
		for (i=D[c]; i!=c; i=D[i])
		{
			remove_repeat(i);
			for (j=R[i]; j!=i; j=R[j]) remove_repeat(j);
			dfs(n+1);
			for (j=L[i]; j!=i; j=L[j]) resume_repeat(j);
			resume_repeat(i);
		}
		return 0;
	}
